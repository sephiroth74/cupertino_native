...
const double _kDefaultSecureTextFieldWidth = 120.0;
const double _kDefaultSecureTextFieldHeight = 24.0;

/// A native macOS secure text field backed by NSSecureTextField.
class CNSecureTextField extends StatefulWidget {
  /// Creates a native secure text field.
  const CNSecureTextField({
    super.key,
    this.controller,
    this.placeholder,
    this.textColor,
...
class _CNSecureTextFieldState extends State<CNSecureTextField> {
  ...
  @override
  void didUpdateWidget(covariant CNSecureTextField oldWidget) {
  ...
  void _onPlatformViewCreated(int id) {
    final channel = MethodChannel('CupertinoNativeSecureTextField_$id');
    _channel = channel;
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        final width =
            widget.width ?? _intrinsicWidth ?? (constraints.hasBoundedWidth ? constraints.maxWidth : _kDefaultSecureTextFieldWidth);
        final height = _intrinsicHeight ?? (constraints.hasBoundedHeight ? constraints.maxHeight : _kDefaultSecureTextFieldHeight);
